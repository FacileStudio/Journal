...
var authModule = Module{
	Name:        "auth",
	Description: "Dashboard accounts. Passwords are Argon2id; a session is a random 32 byte token stored SHA-256 hashed and valid for 30 days, sent as Authorization: Bearer <token>.",
	Routes: []Route{
		{
			Method:       "GET",
			Path:         "/auth/config",
			Summary:      "Read the login screen's configuration",
			Description:  "sso_only and oidc_enabled are constant false — Journal has no OIDC client, the shape exists so the login screen matches the rest of the suite.",
			ResponseBody: `{"sso_only":false,"oidc_enabled":false,"allow_registration":bool}`,
		...
		},
		{
			...
			Description:  "Idempotent: a missing or unknown token is still 200.",
			Auth:         "bearer",
